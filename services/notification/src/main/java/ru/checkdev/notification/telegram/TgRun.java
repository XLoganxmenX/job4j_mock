...
public class TgRun {
    ...
    public TgRun(TgAuthCallWebClint tgAuthCallWebClint, TgAccountRepository accountRepository) {
    ...
    public void initTg() {
        Map<String, Action> actionMap = Map.of(
                "/start", new InfoAction(List.of(
                        ...
                        "/check - вывести информацию, привязанную к аккаунту",
                ...
                "/bind", new BindAction(tgAuthCallWebClint, accountRepository),
                "/unbind", new UnbindAction(accountRepository),
                "/check", new CheckAction(accountRepository)
        );
        try {
